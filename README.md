...
**Install is the whole setup.** Fresh machine: install Guru, run `guru`. It boots bare — no connected model, no roles — and it *knows* it: the briefing shows exactly what's missing. Providers whose keys already exist in your environment (or in the encrypted guru vault) simply light up; for plan providers, `/login <provider>` walks you through that provider's own flow. Minutes after install, Guru is talking to the models your keys unlock.
...
- **`/model`** — the 103-route catalog across 21 providers (direct API keys, operator plan auth, local models); credential presence by env NAME only — values are never read or printed
- **`/login` / `/accounts` / `/logout` / `/keys`** — layered credential resolution (env NAME → the **encrypted guru vault** → optional config template → provider-ecosystem cache). `/keys` (+ `guru keys set <NAME>`) saves a key to the **AES-256-GCM vault** (`~/.guruharness/vault.enc`) as an env-var alternative — for keys that can't live in the shell environment — and it lights up its provider on launch just like env
...
| Provider plan/subscription auth, governed | the session's write approval maps to the delegate CLI's sandbox tiers (`read-only`/`workspace-write`); the danger tier is unreachable by design |
...
| Mandates + YOLO permission model | Read-only floor → YOLO → deny-wins → hard-edge escalation → grants; the constitution survives YOLO |
