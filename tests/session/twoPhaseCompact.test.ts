...
} from '../../src/session/twoPhaseCompact.js';
...
describe("twoPhaseCompact (end-to-end)", () => {
  ...
  it("twoPhaseCompact flattens chat history through both phases", () => {
    const messages: ChatTurnMessage[] = [
      { role: "user", content: "hi" },
      { role: "assistant", content: "hello" }
    ];
    const out = twoPhaseCompact(messages, "greeted", { maxToolResultChars: 200 });
    expect(out[0]?.content.startsWith(SUMMARY_ENTRY_PREFIX)).toBe(true);
