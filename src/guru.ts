...
import { getSharedTodoBoard, resetSharedTodoBoard } from "./tools/builtins/todoTools.js";
...
async function cmdNew(state: GuruState): Promise<void> {
  ...
  resetTurnEngine(state);
  // The session todo board (todo_write/todo_list) is bound to the conversation,
  // not the operator session — `/new` clears it so the new branch starts clean.
  // Without this it leaked across the boundary and confused the model's plans.
