...
/**
 * F317 — provider profile credentials (R-OH-PROF-CRED).
 *
 ...
 * value itself. Values are resolved at call time through {@link resolveSecretHandle}
 ...
 * PRESENCE only; a structural `secretValue: z.never()` guard rejects any literal
 * secret value placed on a profile record.
 */
...
export const ProfileCredentialStoreSchema = z
  .object({
...
export type ProfileCredentialStoreSnapshot = z.infer<typeof ProfileCredentialStoreSchema>;
...
export interface ProfileCredentialStore {
  ...
  readonly snapshot: ProfileCredentialStoreSnapshot;
...
export interface CreateProfileCredentialStoreOptions {
...
export function createProfileCredentialStore(options: CreateProfileCredentialStoreOptions): ProfileCredentialStore {
  ...
  const profilesById = new Map<string, ProviderProfile>(parsed.profiles.map((profile) => [profile.id, profile]));
...
/**
 * Resolve ONLY the active profile's secret handle. Returns `undefined` when the
 * active profile has no usable secret (missing-credential). Crucially, this
 * resolves exactly one profile's reference and returns exactly that handle —
 * activating B can never yield A's key.
 *
 * Kept as a module-level function for parity with the build-plan signature
 * `resolveActiveCredentials(store, activeProfileId)`; the store always honors its
 * own active id, so the second argument is accepted but ignored when omitted.
 */
export function resolveActiveCredentials(
  store: ProfileCredentialStore,
  activeProfileId?: string
): ActiveProfileCredentials | undefined {
  if (activeProfileId !== undefined && activeProfileId !== store.activeProfileId) {
